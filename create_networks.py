...
def get_csv_network():
    try:
        print('=======================Collecting Networks information from CSV file=====================')
        csv_networks_list = []
        csv_networks = open('networks.csv', 'r')
        networks = csv.DictReader(csv_networks, delimiter=',')

        for row in networks:
            csv_networks_list.append(row)
        json_networks = json.loads(json.dumps(csv_networks_list))
        pprint(json_networks)
